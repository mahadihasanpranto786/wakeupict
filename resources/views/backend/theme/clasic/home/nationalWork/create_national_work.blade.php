    ...
    <div class="row">
        <div class="col-md-6">
            <div class="card">
                ...
                <form action="{{ route('store-national-work') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input type="text" name="title" id="title" class="form-control" placeholder="Title"
                                value="{{ old('title') }}" data-validation='required'>
                            @error('title')
                            ...
                            <select name="blog_id" data-validation='required' class="form-control select2"
                                ...
                                @foreach ($blogs as $blog)
                                    <option value="{{ $blog->id }}"
                                        {{ collect(old('blog_id'))->contains($blog->id) ? 'selected' : '' }}>
                            ...
                            <select name="logo" data-validation='required' class="form-control select2"
                                ...
                                @foreach ($icons as $icon)
                                    <option value="{{ $icon->icon }}" data-icon="{{ $icon->icon }}"
                                        {{ collect(old('logo'))->contains($icon->icon) ? 'selected' : '' }}>
                                        {{ $icon->icon }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <textarea type="text" name="description" class="form-control textarea" cols="30" rows="3">{{ old('description') }}</textarea>
                            @error('description')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
