...
@section('national_active', 'active')
{{-- menu active end --}}
...
@section('maincontant')
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header p-2">
                    ...
                    @if (checkUserType() == 0)
                        <a class="float-right" href="{{ route('add-national-work') }}">
                            <button class="btn btn-primary  align-top"><i class="fas fa-plus-circle"></i>
                                Add National Work</button>
                        </a>
                    @endif
                </div>
                <!-- /.card-header -->
                            ...
                            <table class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    @php
                                        $serial = 1;
                                    @endphp
                                    @foreach ($national_works as $project)
                                        <tr>
                                            <td> {{ $serial++ }}</td>
                                            <td>
                                            ...
                                            @if (checkUserType() == 0)
                                                <td class="d-flex">
                                                    ...
                                                    <button title="Delete" class="btn btn-danger btn-sm"><a
                                                            ...
                                                            href="{{ 'delete-national-work/' . $project->id }}"><i
    ...
    <!-- Modal -->
    <div class="modal fade" id="national_work" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header bg-success">
                    <h5 class="modal-title" id="exampleModalLabel">Update Header of National Work</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="{{ route('national-work-header-update') }}" method="POST">
                        @csrf
                        <input type="hidden" name="id" value="{{ $nationalWorkHeader->id }}">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="title" placeholder="Title" id="title"
                                value="{{ $nationalWorkHeader->title }}">
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea type="text" class="form-control textarea" id="description" name="description" placeholder="Description">{{ $nationalWorkHeader->description }}</textarea>
                        </div>
