...
@section('maincontant')
    <div class="row">
        <div class="col-md-6">
            <div class="card">
                <div class="card-header">National Work</div>
                <form action="{{ route('update-national-work') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="id" value="{{ $national_data->id }}">
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input type="text" name="title" id="title" class="form-control" placeholder="Title"
                                value="{{ $national_data->title }}" data-validation='required'>
                            @error('title')
                            ...
                            <select name="blog_id" data-validation='required' class="form-control select2"
                                ...
                                @foreach ($blogs as $blog)
                                    <option value="{{ $blog->id }}"
                                        {{ $blog->id == $national_data->blog_id ? 'selected' : '' }}>{{ $blog->blog_title }}
                                    </option>
                                @endforeach
                            </select>
                            @error('blog_id')
                            ...
                            <select name="logo" data-validation='required' class="form-control select2"
                                ...
                                @foreach ($icons as $icon)
                                    <option value="{{ $icon->icon }}"
                                        {{ $icon->icon == $national_data->logo ? 'selected' : '' }}
                        ...
                        <div class="form-group">
                            ...
                            <input type="file" class="form-control" name="image" id="inputImg" onchange="preview()"
                                placeholder="image" value="{{ $national_data->image }}">
                        ...
                        <div>
                            <input type="hidden" name="old_img" value="{{ $national_data->image }}">
                            <p id="para">Old Image</p>
                            <img width="250px" id="old_img" style=" border: 2px solid gray; padding: 20px;"
                                class="old_img" src="{{ asset($national_data->image) }}" alt="old image">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="image_alt" id="image_alt" class="form-control"
                                placeholder="Enter Image Alt" value="{{ $national_data->image_alt }}"
                        ...
                        <div class="form-group">
                            ...
                            <a type="button" data-toggle="modal" data-target="#development_description"
                                style="float: right; border-radius: 3px;" class="btn-success text-white">Preview</a>
                            <textarea type="text" name="description" class="form-control textarea" cols="30" rows="3">{{ $national_data->description }}</textarea>
                            @error('description')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
