...
@section('maincontant')
    <form action="{{ route('update-blog') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <input type="hidden" name="old_title" value="{{ $blogs->blog_title }}">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="blog_title" id="blog_title" data-validation='required'
                                class="form-control" placeholder="Enter Blog Title" value="{{ $blogs->blog_title }}">
                            @error('blog_title')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="image_alt" id="image_alt" data-validation='required'
                                class="form-control" placeholder="Enter Image Alt" value="{{ $blogs->image_alt }}">
                            @error('image_alt')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="d-none">
                            ...
                            <input type="text" name="templete_name" id="templete_name" class="form-control"
                                placeholder="Course Title" value="{{ $blogs->templete_name }}">
                            @error('templete_name')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input id="headerImg" onchange="previewheader()" name="header_image" type="file"
                                class="form-control" value="{{ $blogs->header_image }}">
                            @error('header_image')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div>
                            <input type="hidden" name="old_img_header" value="{{ $blogs->header_image }}">
                            <p id="para_header">Old Header Image</p>
                            <img width="250px" id="old_img_header" style=" border: 2px solid gray; padding: 20px;"
                                class="old_img_header" src="{{ asset($blogs->header_image) }}" alt="old image">
                        ...
                        <div class="form-group">
                            <label for="blog_image">Thumbnail</label><a href="javascript:;" data-toggle="modal"
                                ...
                                class="btn-success">Preview</a>
                            <input type="file" name="blog_image" id="inputImg" onchange="preview()" class="form-control"
                                placeholder="image" value="{{ $blogs->blog_image }}">
                            @error('blog_image')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        <div>
                            <input type="hidden" name="old_img" value="{{ $blogs->blog_image }}">
                            <p id="para">Old Thumbnail</p>
                            <img width="250px" id="old_img" style=" border: 2px solid gray; padding: 20px;"
                                class="old_img" src="{{ asset($blogs->blog_image) }}" alt="old image">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="slug_title">Slug Title(url)</label><a href="javascript:;" data-toggle="modal"
                                ...
                                class="btn-success">Preview</a>
                            <input type="text" name="slug_title" id="slug_title_edit" class="form-control"
                                placeholder="Slug Title" value="{{ $blogs->slug_title }}" data-validation='required'>
                            @error('slug_title')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <input type="date" name="update_time"
                                ...
                                data-single-mode="true" placeholder="Update Time" value="{{ $blogs->update_time }}"
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="footer_title" id="footer_title" class="form-control"
                                placeholder="Entar footer Title" value="{{ $blogs->footer_title }}"
                                data-validation='required'>
                            @error('footer_title')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="creator_name" id="creator_name" class="form-control"
                                placeholder="Enter Crator name" value="{{ $blogs->creator_name }}"
                                data-validation='required'>
                            @error('creator_name')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <textarea type="text" name="short_description" class="textarea form-control" cols="30"
                                rows="3">{{ $blogs->short_description }}</textarea>
    ...
    <script>
        function previewheader() {
            ...
            if (file) {
                reader.readAsDataURL(file);
                document.querySelector("#old_img_header").removeAttribute('src');
                document.querySelector("#old_img_header").removeAttribute('alt');
                document.querySelector("#old_img_header").removeAttribute('style');
                document.querySelector('#para_header').innerHTML = 'New image';
    ...
    <script>
        $("#slug_title_edit").blur(function() {

            var slug = $('#slug_title_edit').val();
            var blog_id = $('#id').val();

            var slug_lowar = slug.toLowerCase();
            const blog_slug = slug_lowar.replace(/\s/g, '-');
            ...
            if (blog_slug) {
                ...
                $.ajax({
                    ...
                    url: "{{ url('/blog-slug-edit') }}/" + blog_slug + '/' + blog_id,
