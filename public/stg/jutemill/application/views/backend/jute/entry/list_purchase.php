				...
				<div class="col-12">
					<div class="card card-info mt-3">
						<div class="card-header">
							<h3 class="card-title"><i class="fa fa-th"></i> All Purchases</h3>
							<?php
									...
									?>
											<tr>
												...
												<td class="align-middle">
													<?php
													$outturnInfoById = $this->M_jute_entry->getJuteOutTurnSummaryById($list->jpis_ot_id);
													echo date("d-m-Y", strtotime($outturnInfoById->ot_ass_date)); ?>
												</td>
												...
												<td class="text-right align-middle">
													<div class="btn-group py-0 btn-group-sm">
														...
														$approvedDoneInvoice = $this->Common->get_single_row_information_multi_conditional('jute_purchase_invoice_summary', ['jpis_id' => $list->jpis_id, 'jpis_approve_status' => 1, 'jpis_status' => 1]);
														// If user is a invoice creator, no need to approve invoice
														$createdUserInfo = $this->Common->get_single_row_information_multi_conditional('authority', ['a_id' => $list->jpis_created_by, 'a_status' => 1]);
														...
														<button class="btn btn-secondary btn-sm m-1 <?php echo ($approvedDoneInvoice) ? true : "d-none"; ?>"><i class="fas fa-check-circle"></i>Approved</button>

														<?php if ($createdUserInfo) {
															if ($createdUserInfo->a_id == $userId) { ?>

																<button type="button" class="btn btn-warning btn-sm m-1 invoiceReturnInfoButton <?php echo ($jutePurchaseInvoiceReturnInfoByInvoiceId) ? true : "d-none"; ?>" data-invoice-id="<?= $list->jpis_id ?>"><i class="fa fa-info-circle text-white" aria-hidden="true"></i> Return Info </button>
															...
															<?php
																...
																if ($purchaseInvoiceApproveInfoByUser) {
																	if ($purchaseInvoiceApproveInfoByUser->iajp_approval_status == $list->jpis_user_serially_approval_status) { ?>
																		<a href="<?php echo base_url(); ?>jute/Entry/viewPurchaseInvoice?jpis_id=<?= $list->jpis_id ?>"><button class="btn btn-info btn-sm m-1" type="button" data-placement="top" title="View"><i class="fas fa-eye"></i> View Bill Book</button></a>
																		<?php } else {
																		$serialApprove = (int)$list->jpis_user_serially_approval_status + 1;
																		...
																		if ($serialMax == $serialApprove) {	?>
																			...
																			<?php if ($jutePurchaseInvoiceReturnInfoByInvoiceId) { ?>
																				<button class="btn btn-secondary btn-sm m-1"><i class="fas fa-undo-alt"></i> Returned</button>
																				<a href="<?php echo base_url(); ?>jute/Entry/viewPurchaseInvoice?jpis_id=<?= $list->jpis_id ?>"><button class="btn btn-info btn-sm m-1" type="button" data-placement="top" title="View"><i class="fas fa-eye"></i> View Bill Book</button></a>
																			<?php } else { ?>
																				<a href="<?php echo base_url(); ?>jute/Entry/viewPurchaseInvoice?jpis_id=<?= $list->jpis_id ?>"><button class="btn btn-primary btn-sm m-1" type="button" data-placement="top" title="View"><i class="fas fa-eye"></i> Approve Bill Book</button></a>
																			<?php }
																			?>
																			...
																			if ($purchaseInvoiceApproveInfoByUser->iajp_approval_status < $list->jpis_user_serially_approval_status) { ?>
																				<a href="<?php echo base_url(); ?>jute/Entry/viewPurchaseInvoice?jpis_id=<?= $list->jpis_id ?>"><button class="btn btn-info btn-sm m-1" type="button" data-placement="top" title="View"><i class="fas fa-eye"></i> View Bill Book</button></a>
																			<?php } else if ($jutePurchaseInvoiceReturnInfoByInvoiceId) { ?>
																				<button type="button" class="btn btn-warning btn-sm m-1 invoiceReturnInfoButton <?php echo ($jutePurchaseInvoiceReturnInfoByInvoiceId) ? true : "d-none"; ?>" data-invoice-id="<?= $list->jpis_id ?>"><i class="fa fa-info-circle text-white" aria-hidden="true"></i> Return Info </button>
																				<a href="<?php echo base_url(); ?>jute/Entry/viewPurchaseInvoice?jpis_id=<?= $list->jpis_id ?>"><button class="btn btn-info btn-sm m-1" type="button" data-placement="top" title="View"><i class="fas fa-eye"></i> View Bill Book</button></a>
																<?php } else {
																				echo "<span class='btn btn-indigo'><i class='fas fa-bell'></i> Coming...</span>";
																			}
																		}
																	}
																} ?>
														...
														<!-- </?php if ($list->jpis_approve_status == 0) { ?>
															<a href="<?php echo base_url(); ?>jute/Entry/viewPurchaseInvoice?jpis_id=<?= $list->jpis_id ?>"><button class="btn btn-warning btn-sm" type="button" data-placement="top" title="View"><i class="fas fa-eye"></i> Approve Bill Book</button></a>
														</?php } ?> -->



														<!-- </?php if ($list->jpis_approve_status == 1) { ?>
															<a href="<?php echo base_url(); ?>jute/Entry/viewPurchaseInvoice?jpis_id=<?= $list->jpis_id ?>"><button class="btn btn-info btn-sm" type="button" data-placement="top" title="View"><i class="fas fa-eye"></i> View Bill Book</button></a>
														</?php } ?> -->
														<!-- <a href="#"><button class="btn btn-danger btn-sm" type="button" data-placement="top" title="Delete"><i class="fa fa-trash"></i> Delete</button></a> -->
													</div>
												</td>
											</tr>
									<?php }
