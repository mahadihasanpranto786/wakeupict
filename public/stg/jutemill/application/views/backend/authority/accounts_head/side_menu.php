		...
		<nav class="mt-2">
			<ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
				...
				<li class="nav-item has-treeview <?= active_open('dashboard', $main_nav); ?>">
					<a href="<?php echo base_url('accounts_head') ?>" class="nav-link <?= active_nav('dashboard', $main_nav); ?>">
				...
				<li class="nav-item has-treeview <?= active_open('invoice_status', $main_nav); ?>">
					<a href="#" class="nav-link <?= active_nav('invoice_status', $main_nav); ?>">
						...
						if ($purchaseInvoiceApproveInfoByUser) {
						   ...
						   ) AND jpis_user_serially_approval_status = $purchaseInvoiceApproveInfoByUser->iajp_approval_status AND jpis_approve_status = 0 AND jpis_status = 1")->row()->total;
						} else {
